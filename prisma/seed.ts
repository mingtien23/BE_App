...
async function main() {
  // Clean existing test data (order matters for relations)
...
// 1 so dl thì tạo file seed run cái file seed để Backup như admin mất qquyeenf thì run lại 
// -- sêd này sẽ backup dữ liệu hoạc cấu truc database bi thay đổi 
//API làm từng bước cho nó chạy FE test trc đã rồi mới ghi cái middleware phân quyền
// check lại hàm bcrypt xem có đúng không nên chuyển sang hàm #